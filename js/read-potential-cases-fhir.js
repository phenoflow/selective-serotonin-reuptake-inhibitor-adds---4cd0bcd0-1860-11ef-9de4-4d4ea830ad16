...
(async () => {
  ...
  await fs.writeFile("selective-serotonin-reuptake-inhibitor-adds-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("selective-serotonin-reuptake-inhibitor-adds-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
